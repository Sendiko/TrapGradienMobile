...
class ClustersViewModel(
    ...
    fun onEvent(event: ClustersEvent) {
        when (event) {
            ClustersEvent.OnLoadData -> fetchClusters()
            ClustersEvent.OnRefresh -> refresh()
            is ClustersEvent.OnClusterExpand -> toggleExpand(event.cluster.cluster.clusterId)
        }
    }
    ...
    private fun toggleExpand(clusterId: String) {
